...
{assign var="count_content_list" value="0"}		
	...
	<tr>
		<td style="width:25px;"><input type="checkbox" name="box[]" value="{$user.id}" onclick="mycnop();" id="mycheckbox-{$count_content_list}"></td>

		<td style="" class="admin-derr-31" colspan="2">
		<a href="index.php?mod=users&task=edit&id={$user.id}">
		<span class="admin-derr-31">{$user.name}</span></a></td>
		
		<td style="text-align:right; width:150px;" colspan="2" class="admin-derr-37">{$user.group_name}</td>
...
<div style="padding-top:0px;">{$page_nav}</div><br/><br/>	
